...
-- Permette al sito clienti di segnare un proprio ordine come annullato,
-- senza mai farlo sparire dal gestionale staff (nessuna policy di delete
-- concessa ai clienti: possono solo aggiornare, non cancellare la riga).
-- Nota: per impedire che il cliente modifichi anche altri campi oltre a
-- "cancelled", valuta di sostituire questa policy con una funzione RPC
-- dedicata (es. cancel_order(order_id)) eseguita con security definer.
create policy "Il cliente annulla il proprio ordine"
